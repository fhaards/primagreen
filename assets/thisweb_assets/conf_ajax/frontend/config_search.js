					...
					success: function (data) {
						...
						if (data == false) {
							searchResultProducts +=
								"<div class='py-5 mx-auto font-semibold text-gray-500'>Ops, Result Not Found. Try other type </div>";
						} else {
							searchResultProducts +=
							...
							searchResultProducts +=
								'<div class="grid gap-5 grid-cols-2 md:grid-cols-5 grid-flow-row auto-col-max overflow-y-scroll py-4 my-2 scrollbar-none" style="max-height:300px;">';
							for (i = 0; i < data.length; i++) {
								getProductUrl = BASE_URL + "store/product-list/detail/" + data[i].id_barang + "/" + data[i].nm_barang;
								searchResultProducts += '<a href="' + getProductUrl + '" class="flex flex-col overflow-y-auto max-h-48 rounded-md w-auto md:w-full py-2 px-4 my-1 border-2 hover:border-gray-800 border-gray-200 hover:bg-gray-50 transition duration-300 ease-in-out">';
